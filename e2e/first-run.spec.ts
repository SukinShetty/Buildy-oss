...
test('first launch opens Settings with 4 recommended providers and Advanced collapsed', async () => {
  ...
  await expect(buildy.main.getByText('OpenAI', { exact: true })).toBeVisible()
  await expect(buildy.main.getByText('Google Gemini', { exact: true })).toBeVisible()
  await expect(buildy.main.getByText('OpenRouter', { exact: true })).toBeVisible()

  // Advanced (local providers) is collapsed by default: toggle visible, options not.
  await expect(buildy.main.getByText('Advanced: run models locally')).toBeVisible()
  await expect(buildy.main.getByText('Ollama', { exact: true })).toHaveCount(0)
  await expect(buildy.main.getByText('LM Studio', { exact: true })).toHaveCount(0)
})

test('no model is selected and no key is stored on a fresh profile', async () => {
  const settings = await buildy.main.evaluate(async () => {
    const api = (window as unknown as { buildy: { loadSettings(): Promise<unknown> } }).buildy
...
test('watching is blocked with the "Choose a model in Settings" message', async () => {
  ...
  const result = await buildy.companion.evaluate(async () => {
