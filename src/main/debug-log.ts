...
// default and only print when MYBUILDY_DEBUG is set, so a production run never
...
export function isDebug(): boolean {
  return !!process.env.MYBUILDY_DEBUG
}

/** Content-bearing log — printed only when MYBUILDY_DEBUG is set. */
export function debugLog(...args: unknown[]): void {
  if (isDebug()) console.log(...args)
}

/** Content-bearing error — printed only when MYBUILDY_DEBUG is set. */
