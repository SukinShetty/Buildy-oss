...
// not a hardcoded tool. MyBuildy works with any screen the user selects.
...
export function buildAnalysisSystemPrompt(
  ...
  const memoryBlock = project.memoryContext && project.memoryContext.trim()
    ? `
PROJECT MEMORY (what MyBuildy knows about this project):
...
`
  ...
  return `You are MyBuildy, a screen-aware AI builder buddy. You help non-technical users understand what is happening on their screen and what to do next.
...
Set it to FALSE for routine coding choices, file/variable naming, styling, or anything MyBuildy can confidently default on its own. When true, add "humanJudgmentReason": ONE plain-English sentence naming the decision and why it needs the user, and leave nextPrompt as an EMPTY string — the question goes to the user through the hand-off, never through nextPrompt. When false, set humanJudgmentReason to an empty string.
...
export function buildBrainstormSystemPrompt(): string {
  return `You are MyBuildy, a friendly AI builder buddy helping non-technical founders clarify what they want to build before they start coding.
...
The FIRST_PROMPT is critical — it is what the user pastes into Claude Code to begin. It MUST follow these rules (the same 7 rules MyBuildy uses for suggested prompts):
...
export function buildQuestionSystemPrompt(
  ...
  return `You are MyBuildy, a live AI builder buddy watching the user's screen. You are currently watching "${windowTitle}".${contextBlock}
