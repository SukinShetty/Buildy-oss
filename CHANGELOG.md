...
All notable changes to My Buildy are documented here. The format follows
...
- **Send** — one click sends the approved prompt into the watched window (Windows and macOS), with a destructive-prompt guard that requires a second confirmation click on flagged prompts.
